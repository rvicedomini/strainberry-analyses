...
def mock_plots(sample):
    ...
    if sample == 'mock3' and nstrains == 2:
        plot_files = [
            f"results/{sample}/assembly_eval/{sample}.ani.barplot.pdf",
            f"results/{sample}/assembly_eval/{sample}.dupratio.barplot.pdf",
            f"results/{sample}/assembly_eval/{sample}_circos.svg",
            ]
    elif sample == 'mock9' and nstrains in [2,3]:
        plot_files = [
            f"results/{sample}/assembly_eval/{sample}_n{nstrains}.ani.barplot.pdf",
            f"results/{sample}/assembly_eval/{sample}_n{nstrains}.dupratio.barplot.pdf",
            f"results/{sample}/assembly_eval/{sample}_n{nstrains}_circos.svg",
            ]
    return plot_files
